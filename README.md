...
**Auto** — a sped-up simulated day. The city hums with cars satisfying schedule-driven demand. HUD shows active rides, completed rides, average trip time, and a live gridlock score. Random obstacles (jaywalkers, parked trucks, manual-driven cars) spawn on the grid and every car senses and reacts — same kind-specific behaviors as in Driver POV. Scheduled surge events (school starts/ends, train arrivals/departures) inject bursts of requests with on-screen announcements. The city also has 2 randomly-placed one-way streets, 4 stoplights, and 4 stop signs — depicted on both the grid and FPV. **Click any active car** to spectate it: a POV opens for that car alongside a mini-map and the sim continues at 1×. Click another car to switch, or **Exit POV** to return.
